...
Este é um projeto de criação de um ambiente de desenvolvimento local seguro, e replicável, utilizando **Terraform**, enquanto a conteinerização, bem como orquestração dos serviços, fica a cargo do **Docker** por meio do **Compose**, todos com política de *Restart Always*. 
A comunicação entre serviços é isolada pelas redes (sendo elas uma rede interna, uma rede externa e uma rede de observabilidade). O acesso externo é controlado pelo **Nginx**, que aqui faz o papel do **Proxy Reverso** . O monitoramento da saúde da aplicação é visualizada pelo **Grafana**, que por sua vez tem como *Metric scraper*, o **Prometheus** ambos também conteinerizados.
...
| Serviço | Rede de Acesso | Acesso Externo(Isto é, de fora do Docker) |
| :--- | :--- | :--- |
...
| **nginx_prometheus_exporter** | `external_network`, `grafana_network` | Não |

### 1. Isolamento de Rede

*   **`external_network` (Rede Externa):** Conecta o `proxy`, `frontend`, `backend` e o `nginx_prometheus_exporter`. É a rede de comunicação entre os serviços que precisam interagir com o Proxy.
*   **`internal_network`:** Apenas o `db` e o `backend` estão nela, além do `postgres_exporter`. O `db` continua isolado do mundo externo.
*   **`grafana_network`:** Conecta `prometheus`, `grafana`, `backend`, `postgres_exporter` e `nginx_prometheus_exporter`. O grafana tem uma rede própria pra evitar o tráfego de métricas das redes de aplicação, para que o Prometheus possa acessar todos os *exporters* sem se isso "polua" a rede das aplicações.

### 2. Monitoramento

*   **Backend (Node.js):** O `backend/index.js` foi modificado do padrão para usar a biblioteca `prom-client` e expor métricas essenciais no endpoint `/metrics`.
*   **PostgreSQL:** O serviço `postgres_exporter` coleta métricas detalhadas do banco de dados e as expõe ao Prometheus.
*   **Proxy (Nginx):** O serviço `proxy` utiliza a imagem padrão `nginx:alpine` e o arquivo `proxy/nginx.conf` foi configurado para expor o Nginx Stub Status no endpoint `/nginx_status`. Este endpoint que é raspado pelo `nginx_prometheus_exporter` que por sua vez converte os status em metricas ultilizáveis pelo prometheus.
*   **Prometheus:** O `monitoring/prometheus/prometheus-cfg.yml` foi configurado para raspar as métricas de todos os *exporters* e serviços.
...
*   A conexão com o DB agora ocorre apenas uma vez (`client.connect()`) durante o *startup* do servidor invés de por requisição, visando estabilidade.
*   As consultas ao banco de dados agora são funções assíncronas (`async/await`).
*   As credenciais estavam como variáveis não definidas, o que sugeria a ideia delas serem defininas no código. Isso foi corrigido para ele utilizar as variáveis de ambiente completas para a conexão com o PostgreSQL (`PGUSER`, `PGPASSWORD`, `PGHOST`, etc.) injetadas no host. Além disso as credenciais do banco de dados são reutilizadas pelo `postgres_exporter` através do `DATA_SOURCE_NAME` no `docker-compose.yml` e são mantidas em um arquivo .env¹
